...
field, or from the microSD card, using the program [PC400](https://www.campbellsci.com/pc400).
...
possible to redo all data processing following step "import" when
...
every 1 s on 2024-04-15 and later to 50 ms, could in principle decrease the variability of the 1 min summaries. However, the averaging period used for data
acquisition was decreased at the same time as the frequency was
increased, to make it possible. Means in the 1 min and 1 h time series are, thus, unlikely to
...
they are acquired first once every 15 min, and later once every 12 min, and 1 h means of 5 or 6 values logged. I expect this change to have reduced variability caused by measurement errors.
...
the WXT536 for means computed on board the sensor was also increased to 10 s, 1 min and 1 h averages should not be affected by this change. Minima and maxima, and standard deviations could, potentially be slightly affected.
:::
